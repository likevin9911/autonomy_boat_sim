...
HCourseController::HCourseController(ros::NodeHandle& nh): nh_(nh)
{
  HCourseController::setupHThrustController();
  ...
  sub_course_ = nh_.subscribe("/course_cmd", 1, &HCourseController::courseCb, this);
  sub_odom_   = nh_.subscribe("/odom", 1, &HCourseController::odomCb, this);

  tf_listener_ = new tf::TransformListener();

  // Make sure thrusters are reset to zero angles on startup: TODO doesn't work :/
  HCourseController::reconfigureThrusters(HThrustSM::THRUST_RECONFIG_STRAIGHT);
}

HCourseController::~HCourseController()
{
...
void HCourseController::setupHThrustController()
{
  bool use_sim_time;
  ...
  thrust_controller_ = new usv_vrx::HThrustController(thrust_config_,
    priority_yaw_range, neg_scale_factor, motor_cmd_limit, station_tolerance_ang);
  ...
  thrust_sm_ = new usv_vrx::HThrustSM(reconfig_duration);
}

void HCourseController::reconfigureThrusters(HThrustSM::THRUST_STATE state)
{
  std_msgs::Float32 msg;

  if (state == HThrustSM::THRUST_RECONFIG_STRAIGHT)
    msg.data = 0;
...
void HCourseController::odomCb(const nav_msgs::Odometry::ConstPtr& msg)
{
...
void HCourseController::enableThrusters(bool enabled)
{
...
void HCourseController::updateController()
{
  float thrust_right, thrust_left; // Thrust values
  ...
  switch (thrust_sm_->getState()) // Get current state machine state
  {
    ...
    case HThrustSM::THRUST_STATION:
      thrust_controller_->getControlSignalStation( // Station keeping control
        thrust_right, thrust_left, sim_time);
      break;
  }
